...
namespace pyr {
    ...
    void Character::load(FILE* file) {
        ...
        // Generate the test character if no file name is given
        if (!file) {
            _currEther = 50; 
            _maxEther = 50; 
            _currVitality = 100; 
    ...
    // State Changers
    void Character::changeVitality(int change) {
    ...
    void Character::changeEther(int change) {
        _currEther += change;
    ...
    // Weapon Accessors
    std::string Character::getEquippedWeaponName() {
        return _weapons[_equippedWeapon]->_name;
    }

    std::string Character::getWeaponName(int index) {
        PYR_ASSERT(index > -1 && index < (int)_weapons.size(),"Bad Index in getWeaponName");
        return _weapons[index]->_name;
    }

    void Character::getEquippedWeaponStats(int& minDmg, int& maxDmg, int& dmgMult, int& acc) {
        getWeaponStats(_equippedWeapon,minDmg,maxDmg,dmgMult,acc);
    }

    void Character::getWeaponStats(int index, int& minDmg, int& maxDmg, int& dmgMult, int& acc) {
    ...
    // Armor Accessors
    std::string Character::getEquippedArmorName(){
        return _armors[_equippedArmor]->_name;
    }

    std::string Character::getArmorName(int index){
        PYR_ASSERT(index > -1 && index < (int)_armors.size(),"Bad Index in getArmorName");
        return _armors[index]->_name;
    }

    void Character::getEquippedArmorStats(int& def, int& dmgDiv) {
        getArmorStats(_equippedArmor,def,dmgDiv);
    }

    void Character::getArmorStats(int index, int& def, int& dmgDiv) {
        def = _armors[index]->_defense;
        dmgDiv = _armors[index]->_damageDiv;
    }

    void Character::equipArmor(std::string name){
